...
from pyspark.sql.types import StructField,StringType, StructType, DoubleType
...
if __name__ == "__main__":
    ...
    print(responseRDD.collect())
    colNames = lines \
        .filter(lambda line: Utils.COMMA_DELIMITER.split(line)[2] == "country") \
        .map(getColNames)

    print(colNames.collect()[0])

    schema= StructType([StructField("country", StringType()), StructField("age_midpoint", DoubleType()), StructField(
        "occupation", StringType()), StructField("salary_midpoint", DoubleType())])

    #responseDataFrame = responseRDD.toDF(colNames.collect()[0])
    responseDataFrame = responseRDD.toDF(schema)
